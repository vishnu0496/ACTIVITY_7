                                 ...
                                 margin = 50)
{
    ...
    legends_items.append("rect")
        ...
        .on("click", function(event, d) {
            ...
            // Toggle selected class for clicked legend in both plots
            d3.selectAll("rect." + selectedClass)
                .classed("selected", isSelected);
            
            // Get all currently selected countries
            const selectedCountries = [];
            legends_items.selectAll("rect.selected")
                .each(d => selectedCountries.push(d));
            
            // Update circles visibility
            d3.selectAll("circle")
                .style("opacity", function(data) {
                    // If no countries selected or current country is selected, show at full opacity
                    return selectedCountries.length === 0 || 
                           selectedCountries.includes(data[colorCol]) ? 1 : 0.2;
                });
            
            // Update legend items opacity
            d3.selectAll(".marginContainer")
                .selectAll("rect")
                .style("opacity", function(category) {
                    // If no countries selected or current category is selected, show at full opacity
                    return selectedCountries.length === 0 || 
                           selectedCountries.includes(category) ? 1 : 0.2;
                });
            
            // If no legends are selected, reset all opacities
            if (legends_items.selectAll("rect.selected").empty()) {
                d3.selectAll("circle").style("opacity", 1);
                d3.selectAll(".marginContainer rect").style("opacity", 1);
            }
        });
    legends_items
        .append("text")
